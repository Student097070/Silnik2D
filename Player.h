...
public:
    float speed;
    Direction dir = Direction::DOWN; // domyúlny kierunek
        ...
        : SpriteObject(), UpdatableObject(), speed(speed)
    {
        ...
        // Wczytanie sprite'Ûw dla kaødego kierunku
    ...
    void loadDirectionSprites() {
        for (int i = 0; i < 8; ++i) {
            ...
            if (!bmp->loadFromFile(filename.str())) {
                cerr << "Nie uda≥o siÍ wczytaÊ " << filename.str() << std::endl;
                delete bmp;
                continue;
            }
    ...
    void setDirectionSprites(Direction newDir) {
        ...
        for (auto& bmp : sprites[dir])
            bitmaps.push_back(bmp.get()); // same wskaüniki, nie unique_ptr
        currentFrame = 0;
